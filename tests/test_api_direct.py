...
class TestGfalClientLibraryHelpers:
    ...
    def test_copy_file_create_parents_preserves_url_query(self):
        client = GfalClient()
        ...
        dst_fs = SimpleNamespace(
            mkdir=MagicMock(),
            open=lambda _path, _mode: nullcontext(dst_buffer),
        )
        dst_url = (
            "https://eospilot.cern.ch//eos/pilot/test/out/file.root?"
            "authz=zteos64:secret&eos.app=gfal"
        )

        with patch("gfal.core.api.fs.url_to_fs", return_value=(dst_fs, dst_url)):
        ...
        dst_fs.mkdir.assert_called_once_with(
            "https://eospilot.cern.ch//eos/pilot/test/out?"
            "authz=zteos64:secret&eos.app=gfal",
            create_parents=True,
        )
