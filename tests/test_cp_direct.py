...
class TestDryRunRecursive:
    ...
    def test_dry_run_recursive_skip_if_same_checksums(self, tmp_path, capsys):
        """--dry-run --skip-if-same should check checksums and show which files
        would be skipped vs. copied."""
        src = tmp_path / "src"
        dst = tmp_path / "dst"
        src.mkdir()
        dst.mkdir()
    ...
    def test_dry_run_single_file_skip_if_same(self, tmp_path, capsys):
        """--dry-run --skip-if-same on a single matching file prints Skip."""
        src = tmp_path / "a.txt"
        dst = tmp_path / "b.txt"
        src.write_bytes(b"data")
    ...
    def test_dry_run_single_file_skip_if_same_no_match(self, tmp_path, capsys):
        """--dry-run --skip-if-same on a non-matching file prints Copy."""
        src = tmp_path / "a.txt"
        dst = tmp_path / "b.txt"
        src.write_bytes(b"new")
        dst.write_bytes(b"old")
