...
# Note: the lightweight fsspec-xrootd adapter is bundled below so the RPM stays
#       aligned with the base Python package metadata. Full XRootD support still
#       requires separate xrootd Python bindings, which are not bundled here.
...
The lightweight fsspec-xrootd adapter is bundled in this RPM, but full XRootD
support still requires separate xrootd Python bindings in the environment.
...
# Bundle the lightweight XRootD fsspec adapter for consistency with the base
# Python package metadata. The actual xrootd Python bindings remain external.
%{__python3} -m pip install \
    --root %{buildroot} \
    --prefix /usr \
    --no-build-isolation \
